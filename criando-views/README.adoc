...
- Funções de Views: Views podem ser funções simples ou classes baseadas em métodos (CBV). As CBVs fornecem uma abordagem orientada a objetos para organizar a lógica da view.
...
{% block content %}
  <div class="container mt-4">
    ...
    <form method="post" action="{% if pessoa_fisica %}{% url 'update_pessoa_fisica' id=pessoa_fisica.id %}{% else %}{% url 'novo_pessoa_fisica' %}{% endif %}">
        ...
        <button class="btn btn-primary" type="submit">{% if pessoa %}Atualizar{% else %}Salvar{% endif %}</button>
    </form>
